...
class TokenInfoDictionary {
  ...
  target_map: Record<number, (number | string)[]>;
  pos_buffer: ByteBuffer;
  constructor() {
    // console.log("TokenInfoDictionary constructor");
  ...
  buildDictionary(entries: any[][]): { [word_id: number]: string } {
    //TODO: type
    var dictionary_entries: Record<number, any> = {}; // TODO: type // using as hashmap, string -> string (word_id -> surface_form) to build dictionary
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      if (!isFinite(left_id) || !isFinite(right_id) || !isFinite(word_cost)) {
        console.log("failed assertion", entry);
      }
  ...
  targetMapToBuffer(): Uint8Array {
    ...
    for (var key in this.target_map) {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        buffer.putInt(values[i] as number);
      }
    }
