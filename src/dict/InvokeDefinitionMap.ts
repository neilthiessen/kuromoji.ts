...
class InvokeDefinitionMap {
   ...
   */
  init(character_category_definition: Array<CharacterClass>) {
  ...
  lookup(class_name: string): number {
    ...
    if (class_id == null) {
      throw new Error("Character class not found: " + class_name);
      // return null;
    }
    return class_id;
  }
  ...
  dump(): string[] {
    ...
    for (let i = 0; i < this.map.length; i++) {
      let character_class = this.map[i];
      result.push(
        [
          character_class.class_id,
          character_class.class_name,
          character_class.is_always_invoke,
          character_class.is_grouping,
          character_class.max_length,
        ].join(","),
      );
    }
    return result;
