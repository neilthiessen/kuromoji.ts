...
describe("Tokenizer static functions", () => {
  test("splitByPunctuation", function () {
    expect(
...
describe("Tokenizer for IPADic", function () {
  let tokenizer: Tokenizer<IpadicFeatures>; // target object
  ...
  beforeAll(async () => {
    ...
    tokenizer = await kuromoji
      ...
      .buildFromBuffers(allBuffers);
  });

  test("Sentence すもももももももものうち is tokenized properly", function () {
  ...
  test("Sentence include unknown words となりのトトロ are tokenized properly", function () {
    var path = tokenizer.tokenize("となりのトトロ");
    expect(path).to.have.length(3);
  });
  test("研究 is not split", function () {
    var path = tokenizer.tokenize("研究");
    expect(path).to.have.length(1);
  });
  test("Blank input", function () {
    var path = tokenizer.tokenize("");
    expect(path).to.have.length(0);
  });
  test("Sentence include UTF-16 surrogate pair", function () {
  ...
  test("Sentence include punctuation あ、あ。あ、あ。 returns correct positions", function () {
